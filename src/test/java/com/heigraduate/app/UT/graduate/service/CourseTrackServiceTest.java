...
class CourseTrackServiceTest {
    ...
    void getMandatoryCourses_shouldNeverLeakElCourseIntoTn_evenAcrossDifferentYears() {
        ...
        when(courseTrackRepository.findByTrackIdAndAcademicYearIdAndObligatoryTrue(el.getId(), year1.getId()))
                .thenReturn(List.of(elYear1));
        when(courseTrackRepository.findByTrackIdAndAcademicYearIdAndObligatoryTrue(tn.getId(), year1.getId()))
                .thenReturn(List.of());
        ...
        when(courseTrackRepository.findByTrackIdAndAcademicYearIdAndObligatoryTrue(tn.getId(), year2.getId()))
                .thenReturn(List.of(tnYear2));
        when(courseTrackRepository.findByTrackIdAndAcademicYearIdAndObligatoryTrue(el.getId(), year2.getId()))
                .thenReturn(List.of());
