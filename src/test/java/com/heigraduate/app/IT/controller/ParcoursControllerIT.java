...
class ParcoursControllerIT extends AbstractIntegrationTest {
    ...
    void shouldRejectDuplicateCodeWithConflict() {
        restTemplate.postForEntity(
                "/api/parcours", new ParcoursRequest("TN", "Transformation Numerique"), ParcoursResponse.class);
    ...
    void shouldRejectBlankCodeWithBadRequest() {
        ResponseEntity<String> response =
                restTemplate.postForEntity("/api/parcours", new ParcoursRequest("", "Sans code"), String.class);
    ...
    void shouldDeactivateThenExcludeFromActiveOnlyListing() {
        ResponseEntity<ParcoursResponse> created =
                restTemplate.postForEntity(
                        "/api/parcours", new ParcoursRequest("EL", "Ecosysteme Logiciel"), ParcoursResponse.class);
        UUID id = created.getBody().id();

        restTemplate.exchange(
                "/api/parcours/" + id + "/deactivate", HttpMethod.PATCH, new HttpEntity<>(null), ParcoursResponse.class);
